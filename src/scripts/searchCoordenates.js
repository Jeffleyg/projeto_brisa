...
const importCoordenates = async () => {
  ...
  const peoples = [];
  const remainsPeoples = [];

  await connect();

  for (let i = 0; i < allPeoples.length; i++) {
    //search for lat_lng field
    const address = `${allPeoples[i]?.endereco}, ${allPeoples[i]?.bairro}, ${
      allPeoples[i]?.municipio
    }, ${allPeoples[i]?.uf}`;
    const coordinates = await _getCoordinates(address);
    ...
    if (coordinates == null) {
      ...
      try {
        await mo.create("peoples", allPeoples[i]);
      } catch (error) {
