...
async function getClientStatistics(clientId, initialDate, finalDate) {
  try {
    ...
    // ticket médio
    const mediumTicketQuery =
      `select (sum(ns.notsai_val_tot) / count(ns.cli_cod))::numeric(15,2) ticket
    ...
    return {
      totaVendas: mediumTicket ? totalSales[0].total : 0,
      totalFaturamento: totalValue ? totalValue[0].total : 0,
      maiorVenda: biggestSell ? biggestSell[0].maior_venda : 0,
      ticketMedio: mediumTicket ? mediumTicket[0].ticket : 0,
