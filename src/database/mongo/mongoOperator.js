...
const redirect = require("./mongoRedirect.js");
...
module.exports.create = async (model, document) => {
  if (!model) {
    throw new Error("unknow model");
  }
...
module.exports.manyInsert = async (model, documents) => {
  ...
  try {
    const response = await redirect[model].insertMany(documents, {
      ordered: false,
      lean: true,
    });

    Logger.trace(
      "operation of many insertions possibly executed successfully.",
    );
...
module.exports.edit = async (model, filter, document) => {
  ...
  const response = await redirect[model].findOneAndUpdate(filter, {
    $set: document,
  }, options);
...
module.exports.del = async (model, filter) => {
  if (!model) {
    throw new Error("unknow model");
  }
